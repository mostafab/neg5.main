...
(function ($, global) {
    ...
    var $gameElements = $refs.$gameElements;
    var $teamElements = $refs.$teamElements;
    var $configurationElements = $refs.$configurationElements;
    var $tournamentElements = $refs.$tournamentElements;
    var $phaseElements = $refs.$phaseElements;
    ...
    $gameElements.$enterGameButton.click(function (e) {
        ...
        if (!wrong) {
            ...
            $("#add-game-message").empty().append("<p style='margin-left:10px; font-size:16px;color:black;'>Adding Game <i class='fa fa-spinner fa-spin'></i></p>");
            $("#updategamediv").empty().append("<p style='margin-left:10px; font-size:16px;color:black;'>Adding Game <i class='fa fa-spinner fa-spin'></i></p>");
            sendGameToServer();
        }
    });
    ...
    $("#submitregistration").click(function () {
        ...
        if (!empty) {
            ...
            if (numTeams >= 1 && numTeams - Math.floor(numTeams) == 0) {
                ...
                $("#tregmessage").empty().append("<p style='margin-left:10px; font-size:18px;'>Enter a valid number of teams.</p>");
            }
        } else {
            $("#tregmessage").empty().append("<p style='margin-left:10px; font-size:18px;'>Fill in all forms, please.</p>");
        }
    });
    ...
    */
    ...
    function rebuildScoresheet(round, scoresheetInfo, pointScheme) {
        var styles = "* { font-family: 'Verdana', sans-serif;} body { padding:50px;" + "} td { padding:2px;} .header { font-weight:bold; } table { font-size: 14px;" + "width: 100%;} table, th, td { border: 1px solid black;} .outer-list .inner-list{display:inline;}";
        ...
        html += "<h3>" + title + "</h3>";
        // Metadata table
        html += "<table><tbody><tr><td class='header'>Team 1</td><td class='header'>Score</td><td class='header'>" + "Team 2</td><td class='header'>Score</td><td class='header'>Round</td><td class='header'>Moderator</td><td class='header'>Packet</td></tr>";
        html += "<tr><td>" + scoresheetInfo.team1.name + "</td><td>" + scoresheetInfo.team1.score + "</td><td>" + scoresheetInfo.team2.name + "</td><td>" + scoresheetInfo.team2.score + "</td><td>" + round + "</td>";
        ...
        html += "<table style='margin-top:50px'><tbody><tr><td class='header' style='text-align:center'>#</td>" + "<td class='header'>Tossup Answers</td><td class='header'>Bonus</td></tr>";
    ...
    function downloadScoresheets(anchor) {
        $.ajax({
            ...
            type: "GET",
            success: function success(rounds, status, xhr) {
                var zip = new JSZip();
    ...
    function downloadStats(anchor) {
        var downloadFileName = $(anchor).attr("data-link") + "_" + $(anchor).attr("data-phase") + $(anchor).attr("data-file");
        $.ajax({
            url: $(anchor).attr("href"),
            type: "GET",
            success: function success(databack, status, xhr) {
        ...
        $.ajax({
            ...
            success: function success(databack, status, xhr) {
                if (databack.newPhase) {
                    ...
                    var tableHTML = "<tr><td><input type='text' data-phase-id='" + databack.newPhase.phase_id + "'" + "value='" + databack.newPhase.name + "' class='form-control nf-input phase-box input-sm'/></td>";
    ...
    function sendTeamToServer(teamInfo) {
        $("#addteammsg").empty().append("<p style='margin-left:10px; margin-right:10px; font-size:16px;color:black'>Editing<i class='fa fa-spinner fa-spin' style='margin-left:10px'></i></p>");
    ...
    function sendGameToServer() {
        $.ajax({
            ...
            success: function success(databack, status, xhr) {
                ...
                $("<p style='margin-left:10px; font-size:16px; color:#009933'>Successfully added game <i class='fa fa-check-circle'></i></p>").hide().appendTo("#add-game-message").fadeIn(300);
                $("#game-list-template").html(databack);
            ...
            error: function error() {
                $("<p style='margin-left:10px; font-size:16px; color:#ff3300'>Couldn't add game<i class='fa fa-times-circle'></i></p>").hide().appendTo("#add-game-message").fadeIn(300);
            },
            complete: function complete() {
    ...
    function changePointSchemeAJAX(pointTypes) {
        $("#pointdivmsg").empty().append("<p style='margin-left:10px; margin-right:10px;color:black;font-size:16px;'>Working...<i class='fa fa-spinner fa-spin' style='margin-left:10px'></i></p>");
        $.ajax({
    ...
    function editTournamentAJAX() {
        ...
        $("#tournament-update-msgdiv").empty().append("<p style='margin-left:10px; margin-right:10px; color:black;font-size:16px;'>Editing<i class='fa fa-spinner fa-spin' style='margin-left:10px'></i></p>");
        $.ajax({
    ...
    function generateCustomStatsAJAX(button) {
        ...
        $("#customstatsdiv").empty().append("<p style='margin-left:10px; margin-right:10px; font-size:16px;'>Generating Stats <i class='fa fa-spinner fa-spin'></i></p>");
    ...
    function addCollaboratorBox(collaborator) {
        ...
        html += "<button class='btn btn-sm nf-red removecollab' style='margin-top:20px' onclick='removeCollabAJAX(this)' data-collabid='" + collaborator.id + "'>Remove</button>";
        html += "</div>";
    ...
    function createPlayerInputField() {
        ...
        $(newInput).attr("class", classes).attr("name", name).attr("placeholder", placeholder).attr("autocomplete", autocomplete).appendTo("#player-dynamic");
        nextPlayerNum++;
    }
