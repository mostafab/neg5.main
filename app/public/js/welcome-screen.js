...
(function ($) {
    ...
    function submitRegistration() {
        $("#registererror").empty().append("<p style='margin-left:10px; font-size:18px;'>Working... <i class='fa fa-spinner fa-spin' style='margin-left:5px'></i></p>");
        $.ajax({
            url: "/register/local",
            method: "POST",
    ...
    function checkRegistrationForms() {
        ...
        var problem = false;
        if (name.length < constants.MIN_USER_LENGTH || name.length > constants.MAX_USER_LENGTH) {
