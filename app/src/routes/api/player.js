import PlayerManager from './../../managers/model-managers/player';
...
export default (app) => {    
    ...
    app.route('/api/t/:tid/players')
        .post(hasToken, accessToTournament, (req, res) => {
            PlayerManager.addPlayer(req.params.tid, req.body.team, req.body.name, req.currentUser)
    ...
    app.route('/api/t/:tid/players/:playerId')
        .put(hasToken, adminAccessToTournament, (req, res) => {
            PlayerManager.editPlayerName(req.params.tid, req.params.playerId, req.body.name)
                .then(result => res.json({result, success: true}))
                .catch(error => res.status(500).send({error}));
        })
        .delete(hasToken, adminAccessToTournament, (req, res) => {
            PlayerManager.deletePlayer(req.params.tid, req.params.playerId)
                .then(result => res.json({result, success: true}))
                .catch(error => res.status(500).send({error}));
        })
