    ...
    app.post("/tournaments/editphases", function (req, res) {
        ...
        tournamentController.editPhases(req.body.tid, req.session.director._id, phases, function (err, unauthorized, tournament) {
            if (err) {
                ...
                res.render("game-list", { tournament: tournament, admin: true }, function (err, gameHTML) {
                    if (err) {
                        console.log(err);
                        res.status(500).end();
                    } else {
                        res.render("team-list", { tournament: tournament, admin: true }, function (err, teamHTML) {
                            if (err) {
                                console.log(err);
                                res.status(500).end();
                            } else {
